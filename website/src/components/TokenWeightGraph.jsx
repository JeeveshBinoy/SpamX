    ...
    return (
        <div className="w-full">
            ...
            <div className="bg-[#0f172a]/40 backdrop-blur-3xl border border-white/5 rounded-3xl p-8 shadow-[0_32px_64px_rgba(0,0,0,0.5)] relative overflow-hidden group/graph">
                { }
                <div className="absolute top-0 bottom-0 left-[calc(110px+((100%-165px)/2))] w-[2px] bg-white/[0.03] z-0 pointer-events-none"></div>
                ...
                <div className="space-y-4 relative z-10">
                    {data.map((item, idx) => {
                        ...
                        return (
                            <div
                                key={idx}
                                className={`group/row flex items-center gap-4 w-full h-7 transition-all duration-300 ${isActive ? 'z-20' : 'opacity-70 grayscale-[0.3]'}`}
                            ...
                            >
                                ...
                                { }
                                <div className="flex-1 h-3.5 relative bg-white/[0.04] rounded-full overflow-hidden border border-white/5 shadow-inner">
                                    <div className="absolute left-1/2 top-0 bottom-0 w-px bg-white/5 z-0"></div>

                                    { }
                                    <div
                                        className={`absolute h-full transition-all duration-700 cubic-bezier(0.16, 1, 0.3, 1) ${isHamSide ? 'left-1/2 bg-gradient-to-r from-emerald-500/20 to-emerald-500' : 'right-1/2 bg-gradient-to-l from-rose-500/20 to-rose-500'}`}
                                        style={{
                                    ...
                                    >
                                        { }
                                        <div className={`absolute inset-0 bg-white/40 transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0'}`}></div>
                                    </div>
                                </div>

                                { }
                                <div className="w-[45px] shrink-0 flex items-center px-1">
                                    <span className={`text-[9px] font-mono font-black transition-all duration-300 ${isActive ? 'opacity-100 scale-110' : 'opacity-0 translate-x-1'}`}>
                                        {item.raw.toFixed(3)}
                                    </span>
                                </div>
                            </div>
            ...
            <div className="mt-6 flex justify-between items-center text-[10px] font-black uppercase tracking-[0.5em] text-slate-500/40 pl-[120px] pr-[60px] select-none">
                <span className="text-rose-500/60 transition-colors group-hover/graph:text-rose-500">SPAM</span>
                <span className="opacity-10">NEUTRAL</span>
                <span className="text-emerald-500/60 transition-colors group-hover/graph:text-emerald-500">HAM</span>
            </div>
        </div>
    );
}
