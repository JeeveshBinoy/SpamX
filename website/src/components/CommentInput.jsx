...
export default function CommentInput({ text, setText, model, setModel, loading, error, onScan }) {
    return (
        <div className="bg-[#0B0F19]/80 border border-slate-800/60 rounded-2xl p-6 backdrop-blur-xl shadow-xl flex flex-col gap-5 h-full">
            ...
            <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Select Model</label>
            ...
            <button
                ...
                className="w-full py-3.5 bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl font-bold uppercase tracking-wider text-sm hover:brightness-110 hover:-translate-y-0.5 active:translate-y-0 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-lg shadow-orange-900/20 text-white"
            >
                {loading ? (
